...
class StopRouteSchedule extends Component {
  render() {
    ...
    console.log(Helpers.cleanScheduleHeadsign(this.props.schedules[0]).stopRouteDirectionSchedules[0].tripHeadsign);
    return (
      <div>
                ...
                >
                  {rds.scheduleStopTimes.map((sst, i) => (
                    <GridListTile
                      ...
                      style={{
                        backgroundColor:
                          this.props.predictions.indexOf(sst.tripId) > -1
                            ? chroma(Schedules[this.props.route].color)
                                .alpha(0.25)
                                .css()
                            : "rgba(255,255,255,1)",
                        textAlign: "center",
                    ...
                    >
                      {i === 0 || moment(rds.scheduleStopTimes[i - 1].arrivalTime).format("a") !== moment(sst.arrivalTime).format("a")
                        ? moment(sst.arrivalTime)
                            .format("h:mma")
                            .slice(0, -1)
                        : moment(sst.arrivalTime).format("h:mm")}
                    </GridListTile>
                  ))}
                </GridList>
              </CardContent>
        ...
        <div style={{ display: "flex", alignItems: "center", background: "white", padding: "0em 1.5em 1em 1.5em" }}>
          ...
          <span
            style={{
              background: chroma(Schedules[this.props.route].color)
                .alpha(0.25)
                .css(),
              fontSize: ".9em",
              marginLeft: ".25em",
